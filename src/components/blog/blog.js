...
function Blog() {
  ...
  return (
    <main className="container mx-auto px-4 py-10 font-[roobert]" id="products">
      <div className="">
        <div className="container mx-auto px-4  text-center  py-10 mb-5 ">
          <h1 className="font-semibold text-5xl py-2">
            Welcome to Truckala, where logistics meets innovation.
          </h1>
          <p className="text-2xl py-2 ">
            We're dedicated to revolutionizing transportation and logistics with
            cutting-edge technology, expert solutions, and a commitment to
            efficiency. Join us in the journey towards seamless logistics and
          ...
          {Tabs_Data.map((item, idx) => {
            return (
              <li
                key={idx}
                className={
                  'p-2 rounded-full px-6 lg:px-8 lg:py-5 md:text-xl text-xs ' +
                  (openTab === item.id
      ...
      {Tabs_Data.map((item, idx) => {
        return (
          <div
            key={idx}
            className={`flex flex-col md:flex-row lg:flex-row xl:flex-row gap-10 justify-between py-10 items-center ${
              openTab === item.id ? 'block' : 'hidden'
            }`}
          >
            <div className="sm:w-[65%] w-full">
              ...
              <ul className="grid sm:grid-cols-3 grid-cols-2 mt-5 gap-4 w-full">
                {item?.features?.map((_item, _idx) => {
                  return (
                    <li key={_idx} className="flex gap-2 items-center">
                      <IoIosArrowDropdownCircle className="text-blue-700 text-xl" />
                      <span>{_item?.list}</span>
                    </li>
                  );
                })}
              </ul>
